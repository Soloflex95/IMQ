...
⚠️ **IMPORTANT**: The application is in **early development**:
...
git push origin develop  # Auto-deploys to Development environment
git push origin main      # Auto-deploys to Production environment
...
- **Triggers**: Push to main/develop, PRs to main, manual workflow dispatch
- **.NET Version**: 9.0.x
...
### Azure DevOps (Alternative/Legacy)

**Pipeline**: `azure-pipelines.yml`

- **Triggers**: Push to main/develop (only for changes in `src/**` or `tests/**`)
- **.NET Version**: 10.x ⚠️ **(outdated, should be updated to 9.x)**
- **Service Connection**: `IMQ-Azure-Connection`
- **Environments**: `IMQ-Dev`, `IMQ-Prod`

**Deployment Targets**:
- Web App: `imq-web-dev` (dev) / `imq-web` (prod)
- API App: `imq-api-dev` (dev) / `imq-api` (prod)

**Key Difference**: Azure DevOps pipeline deploys both Web and API as separate Azure App Services, while GitHub Actions currently deploys only the Web app.
...
*Last Updated: January 10, 2026 | Reflects .NET 9, in-memory prototype phase, GitHub Actions CI/CD*
